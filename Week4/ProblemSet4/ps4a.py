...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    # initialize updatedHand equal to hand    
    updatedHand = {}
    for letter in hand.keys():
...
def isValidWord(word, hand, wordList):
    ...
    # create a dictionary out of word to compare to hand
    for letter in word:
        if letter not in wordDict.keys():
            wordDict[letter] = 1
        else:
            wordDict[letter] += 1    
    
    # compare the two dictionaries to make sure all the letters in word are in hand
    for letter in wordDict.keys():
        if wordDict.get(letter, 0) > hand.get(letter, 0):
            return False
    ...
    while calculateHandlen(hand) != 0: # As long as there are still letters left in the hand:
        ...
        if word == '.': # If the input is a single period:
            ...
            if not isValidWord(word, hand, wordList): # If the word is not valid:
                ...
                print('"{}" earned {} points. Total: {} points'.format(word, getWordScore(word, n), score))# Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
